...
let spirale p q n a ma =
  open_graph "800x800";
  clear_graph ();
  let c = 400. in
  let d = 50. in
  (*pa+qb=2pi*)
  let b = (2.*.pi -. (float p)*.a)/.(float q) in
  Format.printf "a=%.2f b=%.2f \n" a b;
  let mb = exp(-.(log ma)*.(float p)/.(float q)) in 
  draw_circle (truncate c)  (truncate c) (truncate (d));
  for i = 0 to n*q - 1 do for j = 0 to n*p - 1 do
    let s = (pow ma i)*.(pow mb j) in
    if s >= 1. then (
       let x = s*.cos((float i)*.a+.(float j)*.b) in
       let y = s*.sin((float i)*.a+.(float j)*.b) in
       fill_rect (truncate (c+.d*.x))
	    (truncate (c+.d*.y))
            2 2);
  done; done;
